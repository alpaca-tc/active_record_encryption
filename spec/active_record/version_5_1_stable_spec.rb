...
RSpec.describe 'ActiveRecord::Version51Stable' do
  ...
  describe 'compatible' do
    ...
    example 'saved_changes returns a hash of all the changes that occurred', skip: 'reason: default values are defined as `user provided` instead of `from database`' do
      person = Person.create!(first_name: 'Sean', gender: 'M')
    ...
    example 'changed? in after callbacks returns true but is deprecated', skip: 'reason: inherit from base class instead of Person' do
      klass = Class.new(ActiveRecord::Base) do
        self.table_name = 'people'
